...
export default function ProfessorConferenceVideosPage() {
  ...
  const videosQuery = useQuery({
    queryKey: ["professor-videos", token],
    queryFn: () => getProfessorVideos(token!),
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (result) => {
      if (result.success) {
        ...
        queryClient.invalidateQueries({ queryKey: ["professor-videos"] });
      } else {
  ...
  return (
    <Box>
      ...
      {videosQuery.isLoading ? (
        ...
        <Alert severity="info">
          You haven&apos;t uploaded any conference videos yet. Upload videos
          from conferences you participated in to share with students.
        </Alert>
      ) : (
        <Grid container spacing={3}>
          {videos.map((video) => (
