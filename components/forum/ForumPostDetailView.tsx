...
export function ForumPostDetailView({
  ...
  const isAuthor = userId && post && post.authorId === userId;
  const isAdmin =
    userRole === AuthRole.Admin || userRole === AuthRole.EventOffice;
  const isPinned = post?.status === ForumPostStatus.PINNED;
  const isClosed = post?.status === ForumPostStatus.CLOSED;
  const isPostAdminAuthor = post?.authorType === "Admin";
...
function AnswerCard({
  ...
  const isAdminAuthor = answer.authorType === "Admin";
          ...
          <Box flex={1}>
            ...
            >
              <Stack direction="row" alignItems="center" spacing={1}>
                ...
                <Typography variant="caption" color="text.secondary">
                  {answer.authorName} · {formatRelative(answer.createdAt)}
                </Typography>
              </Stack>
              <Stack direction="row" spacing={1}>
...
function CommentItem({
  ...
  const isAdminAuthor = comment.authorType === "Admin";
