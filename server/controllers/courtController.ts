...
export class CourtController {
  viewAllCourts = async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(500)
        .json({ success: false, message: "Failed to fetch courts" });
    }
  };

  viewCourtAvailability = async (req: AuthRequest, res: Response) => {
    try {
  ...
  reserveCourt = async (req: AuthRequest, res: Response) => {
    try {
      const { courtId } = req.params;
